...
make -C rs-worker clean all USE_LIBGMP=1
make -C rs-client clean all

# pushd $MAPDIR
# ./unpack.sh h2esieve-34.lut50 "$TMP"/collatz/src/worker
# popd
# mv worker/h2esieve-34.lut50.map worker/esieve-34.lut50.map

cd rs-client

# 4 hours walltime: limit 2 hours for the mclient; 3 hours for the worker
# 24 hours walltime: 23 hours for the mclient
stdbuf -o0 -e0 ./rs-client -a 10800 -b 82800 4
