#!/bin/bash
#SBATCH --job-name=rs-collatz
#SBATCH --account=OPEN-36-62
...
ssh -TN -f -L 5007:localhost:5007 login1
...
ml GMP
# FIXME
ml Clang/18.1.8-GCCcore-13.2.0-CUDA-12.4.0
...
CC=gcc
#if type clang > /dev/null 2> /dev/null && clang --version | grep -qE "version "; then
